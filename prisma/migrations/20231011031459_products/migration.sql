...
CREATE TABLE `customers_profiles` (
    ...
    UNIQUE INDEX `customers_profiles_user_id_key`(`user_id`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `policies` (
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    `type` ENUM('about', 'new', 'refund', 'how_to_buy', 'contact', 'complain') NOT NULL,
...
-- AddForeignKey
ALTER TABLE `customers_profiles` ADD CONSTRAINT `customers_profiles_user_id_fkey` FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `product_reviews` ADD CONSTRAINT `product_reviews_customer_id_fkey` FOREIGN KEY (`customer_id`) REFERENCES `customers_profiles`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
